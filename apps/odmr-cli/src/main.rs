...
//!
//! 这不是最终产品 CLI，只是把 transport 和 station-resolver 连成一条可执行链。
...
fn usage() -> String {
    [
        ...
        "  odmr run execute --station <path> --calibration <path> --plan <path> --smb-profile <path> --oe-profile <path> [--out-dir <path>]",
        "",
        "示例:",
        ...
        "  odmr run execute --station configs/stations/lab_a.json --calibration configs/calibrations/main.json --plan configs/plans/minimal_3point_runtime.json --smb-profile configs/profiles/smb100a_run_pll_default.json --oe-profile configs/profiles/oe1022d_run_ch_b_observed.json --out-dir runs/manual",
    ]
    .join("\n")
}
