...
OE_CHOICES = {
    ...
    "input_source": [
        ...
        code_choice(2, "1 MOhm 电流输入"),
        code_choice(3, "100 MOhm 电流输入"),
    ],
...
class ConfigGeneratorApp(tk.Tk):
    ...
    def _build_oe_page(self, parent: ttk.Frame) -> None:
        self.oe_vars = {
            ...
            "input_coupling": tk.StringVar(value=oe_choice_for_code("input_coupling", 0)),
            ...
            "time_constant_index": tk.StringVar(value=oe_choice_for_code("time_constant_index", 7)),
        ...
        self._section_title(parent, "OE1022D 固定配置", 0)
        oe_fields: list[tuple[str, str] | tuple[str, str, list[str]]] = [
            ("profile_id", "配置 ID"),
            ("command_settle_ms", "命令等待时间"),
            ("channel", "通道", OE_CHOICES["channel"]),
            ("input_source", "输入信号源", OE_CHOICES["input_source"]),
            ("input_grounding", "输入屏蔽接地", OE_CHOICES["input_grounding"]),
            ("input_coupling", "输入耦合", OE_CHOICES["input_coupling"]),
            ("line_notch_filter", "输入陷波器", OE_CHOICES["line_notch_filter"]),
            ("reference_source", "参考信号源", OE_CHOICES["reference_source"]),
            ("reference_slope", "外部参考触发方式", OE_CHOICES["reference_slope"]),
            ("phase_deg", "参考相位"),
            ("harmonic_1", "谐波 1 检测"),
            ("harmonic_2", "谐波 2 检测"),
            ("dynamic_reserve", "动态储备", OE_CHOICES["dynamic_reserve"]),
            ("sensitivity_index", "满偏灵敏度", OE_CHOICES["sensitivity_index"]),
            ("time_constant_index", "滤波器时间常数", OE_CHOICES["time_constant_index"]),
            ("filter_slope", "滤波器陡降", OE_CHOICES["filter_slope"]),
            ("sync_filter", "同步滤波器", OE_CHOICES["sync_filter"]),
            ("sine_output_mode", "正弦信号输出模式", OE_CHOICES["sine_output_mode"]),
            ("sine_output_voltage_vrms", "正弦输出幅值"),
        ]
        self._form_grid(parent, self.oe_vars, oe_fields, start_row=1, columns=1)
        ttk.Label(
            parent,
            text="采集链路锁定：frame_exact_bytes=12288，rall_post_write_delay_ms=30。此界面不修改 RALL 热路径。",
            wraplength=840,
            foreground="#555555",
