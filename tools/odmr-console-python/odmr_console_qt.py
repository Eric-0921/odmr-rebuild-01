...
class ConfigGeneratorPage(QWidget):
    ...
    def _build_oe(self) -> None:
        ...
        self.oe_fields: dict[str, QWidget] = {
            ...
            "input_coupling": combo(OE_CHOICES["input_coupling"], find_choice_by_code("input_coupling", 1)),
            ...
            "time_constant_index": combo(OE_CHOICES["time_constant_index"], find_choice_by_code("time_constant_index", 9)),
    ...
    def _build_laser(self) -> None:
        ...
        group = QGroupBox("CNI Laser run background profile")
        grid = QGridLayout(group)
        self.laser_profile_id = QLineEdit()
        self.laser_mode = combo(["off_background", "on_background"], "off_background")
        self.laser_power = NumberUnitInput(0, LASER_POWER_UNITS, "mW")
        self.laser_settle = NumberUnitInput(1000, TIME_UNITS, "ms")
        for index, (label, widget) in enumerate([
            ("Profile ID", self.laser_profile_id),
            ("Mode", self.laser_mode),
            ("Power", self.laser_power),
            ("Settle", self.laser_settle),
        ]):
            add_form_row(grid, index // 2, index % 2, label, widget)
        layout.addWidget(group)
        layout.addWidget(note_label("Laser 是 run 级 background profile；C# runtime 在 run 边界开关，不在每个 point 反复 on/off。"))
        layout.addStretch(1)
    ...
    def _set_laser_values(self, laser: dict[str, Any]) -> None:
        ...
        self.laser_mode.setCurrentText(laser.get("mode", "off_background"))
...
class ResolvePage(QWidget):
    ...
    def _summary(parsed: dict[str, Any]) -> str:
        return "\n".join([
            ...
            f"  sweep_points={parsed.get('sweep_estimate', {}).get('sweep_points')}",
            f"  sweep_duration_ms={parsed.get('sweep_estimate', {}).get('sweep_duration_ms')}",
        ])
...
class RunMonitorPage(QWidget):
    ...
    def refresh_progress(self) -> None:
        ...
        if self.latest_progress:
            ...
            if latest.get("state") in {"Completed", "Failed", "CleanupFailed"}:
                self.timer.stop()
                self.start_button.setEnabled(True)
                self.stop_button.setEnabled(False)
                return
