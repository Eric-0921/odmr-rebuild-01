...
**网口状态更新**：复核手册第 7.2 节网口通讯截图后确认，OE1300/1351 Console 软件通过 **TCP Client 连接 `192.168.1.1:10001`**，PC 端应设为 `192.168.1.10/24`。然而当前真机（固件 `Version:V1.3230310`）在 `192.168.1.1:10001` 无响应，串口网络配置命令也超时/不持久化，因此**网口在当前固件上不可用**。

**串口状态更新**：早期探测误以为串口 ~1.5 s/命令，复测显示实际单条命令延迟仅 20–80 ms；100 次连续 `RALL?` 实测平均吞吐 **~18 Hz**。因此第一版**可以基于串口实现**，18 Hz 基本可满足多数 ODMR 实验需求（详见 `docs/equipment_manual/oe1300/clean/probe_reports/20260616_serial_probe.md`）。
