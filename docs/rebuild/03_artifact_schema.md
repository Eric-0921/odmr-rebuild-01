...
```json
{
  ...
  "raw_len": 12288,
  "parse_status": "ok",
  "packet_counter_candidate_u8": 37,
  "packet_counter_gap": 1,
  "duplicate_of": null
}
```

`packet_counter_candidate_u8` / `packet_counter_gap` 来自原厂 `SSI_check packet.vi` 的连续包检查思路。当前只作为采集连续性诊断线索记录，不作为 quality 硬判定，直到真机 run 证明该候选字段稳定按 `+1` / `255 -> 0` 递增。
