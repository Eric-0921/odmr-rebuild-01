...
师兄师姐认可的 LI-ODMR 图只能作为人工判读模板，不能作为固定频率标签。

允许从图中学习的内容：

- 负瓣/正瓣夹着零交叉的局部反对称结构。
- 同一 NV 取向在偏置磁场下可能呈现成对零交叉。
- 人眼看峰时通常同时看平滑曲线、零线、两侧极值和局部斜率。

禁止从图中直接继承的内容：

- 不能把图上的若干 GHz 数值写死为当前样品的 `label`。
- 不能用固定频率表判断“过零点是否找齐”。
- 不能把参考图频率作为训练集监督标签。

当前数据的候选过零点必须从本 run 自己的 `frequency_ghz + b_x/b_y` 形态中产生；跨 run 比较时，只比较结构数量、连续移动趋势、SNR 和 setpoint 对应关系。
...
2026-06-15 对初始阶段跳变的判断：

- 旧策略是 run 级 RF 常开，然后每个 point 设置 `FREQ CW start` 和 sweep 参数。
- 这会让“设置起始频率、切换 sweep mode、扫完回 CW start”的瞬态在 RF 输出打开时真实照射样品。
- 对 LI-ODMR 过零识别来说，这类瞬态容易表现成首段尖峰、边缘跳变或非物理局部结构。

新的采集策略：

1. run 开始后强制 `OUTP OFF`。
2. 每个 point 先在 RF OFF 下设置 `FREQ CW start` 和 sweep 参数。
3. 当前 point 准备进入采集窗口前，打开 RF 输出并切到 `FREQ:MODE SWE`。
4. `segment` 只绑定 `SWE:FREQ:EXEC` 的 sweep execute 窗口。
5. sweep execute 完成后先关闭 RF，再回到 `CW start`。

后处理要求：

- 新旧 RF 时序的 run 不能不加标记地混入同一训练集。
- 旧 run 的首段和尾段要重点看 `edge jump`、局部最大步进和首若干频点是否异常。
- 如果要训练过零点识别，优先使用新 RF 时序后的数据作为主训练集，旧数据可作为鲁棒性或反例样本。
